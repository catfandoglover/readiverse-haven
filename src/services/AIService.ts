...
// Add token tracking to our AIService
class AIService {
  private apiKey: string = '';
  private initialized: boolean = false;
  private apiUrl: string = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent';
  ...
  async generateResponse(
    ...
    try {
      // First check if user has available tokens
      const { data: tokenData, error: tokenError } = await supabase.functions.invoke('get-token-usage');
      
      if (tokenError) {
        console.error("Error checking token availability:", tokenError);
        // Continue anyway to prevent blocking users if the check fails
      } else if (!tokenData.hasAvailableTokens) {
        // User has reached their token limit
        return {
          text: "You've reached your monthly limit of AI conversations. To continue chatting with Virgil, please upgrade to our SURGE subscription for unlimited access.",
      ...
      try {
        ...
        // A better implementation would use the actual token count from the AI response
        const promptTokens = Math.ceil(userMessage.length / 4);
        const responseTokens = Math.ceil(responseText.length / 4);
        const totalTokens = promptTokens + responseTokens;
        
        // Don't track tokens for assessments or if user is already a subscriber
        
        await supabase.functions.invoke('track-token-usage', {
          body: {
