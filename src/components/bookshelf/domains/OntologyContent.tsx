...
const OntologyContent: React.FC = () => {
  ...
  const { data: books, isLoading } = useQuery({
    queryKey: ["ontology-user-books", user?.Account?.Uid],
    queryFn: async () => {
      if (!user?.Account?.Uid) return [];

      try {
        console.log("Fetching ontology books for user:", user.Account.Uid);
        ...
        const { data: booksData, error: booksError } = await supabase
          ...
          .in("id", bookIds)
          .ilike('categories', '%ontology%');
          
        if (booksError) {
          console.error("Error fetching ontology books:", booksError);
          return [];
        }
        ...
        console.error("Exception fetching ontology books:", err);
        return [];
      }
    },
