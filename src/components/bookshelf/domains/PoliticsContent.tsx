...
const PoliticsContent: React.FC = () => {
  ...
  const { data: books, isLoading } = useQuery({
    queryKey: ["politics-user-books", user?.Account?.Uid],
    queryFn: async () => {
      if (!user?.Account?.Uid) return [];

      try {
        console.log("Fetching politics books for user:", user.Account.Uid);
        ...
        const { data: booksData, error: booksError } = await supabase
          ...
          .in("id", bookIds)
          .ilike('categories', '%politics%');
          
        if (booksError) {
          console.error("Error fetching politics books:", booksError);
          return [];
        }
        ...
        console.error("Exception fetching politics books:", err);
        return [];
      }
    },
