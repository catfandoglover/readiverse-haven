...
const AestheticsContent: React.FC = () => {
  ...
  const { data: books, isLoading } = useQuery({
    queryKey: ["aesthetics-user-books", user?.Account?.Uid],
    queryFn: async () => {
      if (!user?.Account?.Uid) return [];

      try {
        console.log("Fetching aesthetics books for user:", user.Account.Uid);
        ...
        const { data: booksData, error: booksError } = await supabase
          ...
          .in("id", bookIds)
          .ilike('categories', '%aesthetics%');
          
        if (booksError) {
          console.error("Error fetching aesthetics books:", booksError);
          return [];
        }
        ...
        console.error("Exception fetching aesthetics books:", err);
        return [];
      }
    },
