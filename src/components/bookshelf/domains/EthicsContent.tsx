...
const EthicsContent: React.FC = () => {
  ...
  const { data: books, isLoading } = useQuery({
    queryKey: ["ethics-user-books", user?.Account?.Uid],
    queryFn: async () => {
      if (!user?.Account?.Uid) return [];

      try {
        console.log("Fetching ethics books for user:", user.Account.Uid);
        ...
        const { data: booksData, error: booksError } = await supabase
          ...
          .in("id", bookIds)
          .ilike('categories', '%ethics%');
          
        if (booksError) {
          console.error("Error fetching ethics books:", booksError);
          return [];
        }
        ...
        console.error("Exception fetching ethics books:", err);
        return [];
      }
    },
